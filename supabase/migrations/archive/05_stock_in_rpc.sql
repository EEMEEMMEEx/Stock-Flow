-- =================================================================
-- MIGRATION 05: ATOMIC STOCK-IN RPC & PROJECT INTEGRITY CONSTRAINTS
-- =================================================================

-- 1. Ensure foreign key constraints on project_id are ON DELETE RESTRICT
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'stock_in_orders_project_id_fkey'
  ) THEN
    ALTER TABLE public.stock_in_orders DROP CONSTRAINT stock_in_orders_project_id_fkey;
  END IF;
  
  ALTER TABLE public.stock_in_orders 
    ADD CONSTRAINT stock_in_orders_project_id_fkey 
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'withdrawal_orders_project_id_fkey'
  ) THEN
    ALTER TABLE public.withdrawal_orders DROP CONSTRAINT withdrawal_orders_project_id_fkey;
  END IF;
  
  ALTER TABLE public.withdrawal_orders 
    ADD CONSTRAINT withdrawal_orders_project_id_fkey 
    FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE RESTRICT;
END $$;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'stock_transactions_project_id_fkey'
  ) THEN
    ALTER TABLE public.stock_transactions DROP CONSTRAINT stock_transactions_project_id_fkey;
  END IF;
  
  ALTER TABLE public.stock_transactions 
    ADD CONSTRAINT stock_transactions_project_id_fkey 
    FOREIGN KEY (project_id) REFERENCES public.projects(id) ON DELETE RESTRICT;
END $$;

-- 2. Create Supabase RPC: process_stock_in
...
BEGIN
  ...
  -- Validate each item in the payload & resolve/create items if item_id is missing
  ...
  LOOP
    ...
    IF v_item_id IS NULL THEN
      ...
      END IF;
      IF v_item_id IS NULL THEN
        INSERT INTO public.items (name, sku, unit)
        VALUES (
  ...
  LOOP
    ...
    -- Re-resolve v_item_id if it was created/resolved in validation step
    IF v_item_id IS NULL THEN
      IF (v_item_json->>'sku') IS NOT NULL AND TRIM(v_item_json->>'sku') != '' THEN
        SELECT id INTO v_item_id FROM public.items WHERE LOWER(sku) = LOWER(TRIM(v_item_json->>'sku')) LIMIT 1;
      END IF;
