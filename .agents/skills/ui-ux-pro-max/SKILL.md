...
> **⚠️ Important for Windows / Git Bash users:**
> All command examples below assume you have already `cd`'d into the `scripts/` directory of this skill:
> ```bash
> cd "d:\APP\Stock-Flow-app\.agents\skills\ui-ux-pro-max\scripts"
> ```
> Run that once per terminal session before invoking any of the commands in the rest of this file.
...
**Recommended — cd into the scripts directory first** (works on Windows / macOS / Linux):

```bash
# From anywhere — full Windows path
cd "d:\APP\Stock-Flow-app\.agents\skills\ui-ux-pro-max\scripts"
python search.py "<query>" --domain <domain>

# macOS / Linux path
cd "/path/to/.agents/skills/ui-ux-pro-max/scripts"
python3 search.py "<query>" --domain <domain>
```

**Or invoke by absolute path** (no cd required):

```bash
# Windows (Git Bash / PowerShell)
python "/d/APP/Stock-Flow-app/.agents/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>

# Windows (cmd / PowerShell native)
python "d:\APP\Stock-Flow-app\.agents\skills\ui-ux-pro-max\scripts\search.py" "<query>" --domain <domain>

# macOS / Linux
python3 "/path/to/.agents/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```
...
```bash
python search.py "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python search.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python search.py "<keyword>" --domain <domain> [-n <max_results>]
```
...
python search.py "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python search.py "search loading animation" --domain ux
